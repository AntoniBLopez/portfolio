...
import { projectCategories, projects, tx, type Project, type ProjectCategory } from "@/content/site";
...
export function ProjectGrid({
  ...
  const availableCategories = useMemo(
    () => projectCategories.filter((category) => items.some((p) => p.category === category.id)),
    [items],
  );
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence mode="popLayout">
            {visible.map((project) => (
              ...
              >
                <ProjectCard project={project} audience={audience} />
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
