...
export async function ServicePackages({
  ...
  cta: {
    label: string;
    external?: boolean;
    /** Shared link for every package, or a builder that receives the localized package name. */
    ...
    >
      ...
      >
        {service.packages.map((pkg) => {
          const packageName = tx(pkg.name, locale);
          const href = typeof cta.href === "function" ? cta.href(packageName) : cta.href;
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <h3 className="text-lg font-semibold text-ink">{packageName}</h3>
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-baseline gap-2">
                    {approximatePricing && (
                      <span className="text-sm font-medium text-ink-3">{t("startingAt")}</span>
                    )}
                  ...
                  {approximatePricing && (
                    <p className="text-xs text-ink-3">{t("priceGuide")}</p>
                  )}
                </div>
